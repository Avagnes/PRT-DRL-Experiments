...
class MDPFuzz(Fuzz):
    def __init__(self, args, SeedSpace, Execute, algo:str = None, model_path:str = None, use_freshness = True):
        super().__init__(args, SeedSpace, Execute, algo, model_path)
        self.guidance = Guidance(use_freshness)
...
class Guidance(GuidanceBase):
    ...
    GMMthreshold = 0.01
    cvg_threshold = 0.01
    def __init__(self, use_freshness=True):
    ...
    def state_coverage(self, states_seq):
        states_seq = np.array(states_seq).reshape(len(states_seq), -1)
        ...
        if GMMpdfvalue < self.GMMthreshold:
            ...
            gamma = 0.01  # modify according to [Policy Testing with MDPFuzz (Replicability Study)](https://dl.acm.org/doi/abs/10.1145/3650212.3680382)
    ...
    def analyze(self, data):
        samplePr = np.abs(data.current_reward - data.last_reward) + 1e-6
        if self.use_freshness:
            cvg = self.state_coverage(data.sequence)
            add = (
                data.current_reward < data.last_reward or
                cvg < self.cvg_threshold
            )
        else:
            add = True
        return samplePr, add