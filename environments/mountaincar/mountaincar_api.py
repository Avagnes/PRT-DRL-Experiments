...
class SeedSpace(SeedBase):
    def __init__(self):
        super().__init__(
            high = np.array([0, 0.05]),
            low = np.array([-1.0, -0.05]),
            shape = (2,),
            dim = 2
        )
...
class Execute(ExecuteBase):
    def __init__(self, seed=42, render=False, algo:str = None, model_path: str = None) -> None:
        if render:
            self.env = gymnasium.make("MountainCarContinuous-v0", render_mode="human")
        else:
            self.env = gymnasium.make("MountainCarContinuous-v0")
        if algo is None:
            algo = "tqc"
        ...
        if model_path is None:
            model_path = f'models/{algo}/MountainCar-v0.zip'
    ...
    def __call__(self, seed):
        ...
        return episode_reward, not terminated, info
...
if __name__ == "__main__":
    ...
    episode_reward, terminated, info = execute(seed)
    print(episode_reward, terminated)
    episode_reward, terminated, info = execute([-0.3,0])
    print(episode_reward, terminated)
